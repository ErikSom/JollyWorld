...
serviceWorkerFiles = files.filter(file => file.indexOf('.woff2') >= 0);

// remove all linked pngs
files = files.filter(file => !pngsToDelete.includes(file) && file.indexOf('.DS_Store') < 0 && file.indexOf('/terms/') < 0 &&  file.indexOf('images/backgrounds') <0 && file.indexOf('/credits/') < 0 && file.indexOf('awesome-game.js') < 0 && file.indexOf('cycling.png') < 0 && file.indexOf('Logo_v1.svg') < 0 && file.indexOf('.woff2') < 0 && file.indexOf('Multiplayer_Character') && path.basename(file).length<30);
