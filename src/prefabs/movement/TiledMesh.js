...
export class TiledMesh extends PIXI.SimpleMesh {
	...
	updateLocalUserData (force = false) {
		...
		if(this.cachedSpriteRotation !== targetSprite.rotation || force){
			...
			for (let i = 0; i < this.verticesClone.length; i += 2){
			...
			const uvs = new Float32Array(this.vertices.length);

			for (let i = 0; i < this.vertices.length; i+=2) {
				uvs[i] = this.vertices[i] * 2.0 / tex.width + 0.5;
				uvs[i + 1] = this.vertices[i + 1] * 2.0 / tex.width + 0.5;
