...
let vehicle1 = new function(){ //Vehicle
        ...
        switch(property){
            case "selectedVehicle":
                if(target.settings.selectedVehicle != value){
                    ...
                    game.editor.applyToObjects(game.editor.TRANSFORM_FORCEDEPTH, depth, newObjects);
	...
	this.contactListener.PostSolve = function (contact, impulse, target) {
        ...
        for(var i = 0; i<bodies.length; i++){
            ...
            for(var j = 0; j<vehicleCharacters.length; j++){
                if(body.mySprite.data.groups.indexOf(vehicleCharacters[j])>=0){
                    characterFunctions.contactListener.PostSolve(contact, impulse, target, vehicleCharacters[j]);
                    foundGroup = true;
                    break;
                }
            }
