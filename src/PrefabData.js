        ...
        this.contactListener.PostSolve = function (contact, impulse) {
            ...
            for(var i = 0; i<bodies.length; i++){
                ...
                if(body == self.lookupObject["head"] && (bodies[0].mySprite.data.prefabID != bodies[1].mySprite.data.prefabID  || bodies[0].mySprite.data.prefabID == undefined)){
                    ...
                    if(force > 8){
                        self.collisionUpdates.push({type:character.GORE_SNAP, target:"eye_right"});
                        self.collisionUpdates.push({type:character.GORE_SNAP, target:"eye_left"});
                        self.collisionUpdates.push({type:character.GORE_SNAP, target:"arm_left"});
                    }
                }
            }
        }
...
class vehicle extends basePrefab{
    ...
    constructor(target){
        super(target);
        Object.assign(vehicle.settings, {"selectedVehicle":"vehicle1"});
        Object.assign(vehicle.settingsOptions, {"selectedVehicle":["vehicle1", "vehicle2"]});
