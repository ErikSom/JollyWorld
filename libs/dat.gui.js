  ...
  var ColorController = function (_Controller) {
    ...
    `;
      ...
      _this2.__colorplus.onmousedown = () => {
        ...
        if (!window.__guiusercolors.includes(colorString)) {
          window.__guiusercolors.push(colorString);
          updateColors();
        }
      }
      ...
      _this2.__colorpicker.onmousedown = () => {
        ...
        editorGUI.style.display = 'none';
        const canvas = document.querySelector('canvas');
        canvas.style.pointerEvents = 'none';
        const blockerElement = document.createElement('div');
        blockerElement.style.width = '100%';
        blockerElement.style.height = '100%';
        blockerElement.style.position = 'absolute';
        blockerElement.style.top = blockerElement.style.left = '0';
        blockerElement.style.zIndex = 9999;
        blockerElement.style.cursor = 'crosshair';
        blockerElement.onmousedown = e=>{
          canvas.style.pointerEvents = 'all';
          blockerElement.parentNode.removeChild(blockerElement);
          editorGUI.style.display = 'block';
          // get pixel color
         window.__pixiScreenshot().then(drawCanvas => {
            const drawCtx = drawCanvas.getContext('2d');
            const p = drawCtx.getImageData(e.clientX, e.clientY, 1, 1).data;
